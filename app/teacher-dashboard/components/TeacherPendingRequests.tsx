...
interface TeacherPendingRequestsProps {
  requests: EnrollmentRequest[];
  setRequests: React.Dispatch<React.SetStateAction<EnrollmentRequest[]>>;
  onChanged?: () => void;
}

function formatDateSafe(raw?: string) {
  if (!raw) return '';
...
export default function TeacherPendingRequests({ requests, setRequests, onChanged }: TeacherPendingRequestsProps) {
  ...
  // Fetch exactly once on mount. We deliberately do NOT depend on requests.length
  // because the parent can momentarily pass an empty array right after an approve/reject,
  // which would otherwise re-trigger this effect and cause an infinite refetch loop.
  useEffect(() => {
  ...
  const handleAction = async (requestId: number, action: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Side effects: refresh server-truth for both lists so sidebar count and any other consumers stay accurate
        onChanged?.();
      } else {
              ...
              >
                ...
                <div className="flex items-start gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-slate-100 font-semibold truncate">{request.studentName || 'Unknown Student'}</p>
